...
<body id="contact" class=" relative"> 
    ...
    <div class=" emag-header pt-10 h-[250px] lg:h-[400px] relative">
        ...
        <div class="container relative mx-auto px-10 py-10 flex items-center w-full h-full justify-center">
            <h1 class="text-white text-center text-2xl uppercase md:text-2xl border-b"> <span class="font-bold ">Magazine Subscribers </h1>
        </div>
    </div>
    <main>

        <div class="container mx-auto px-4 overflow-auto lg:px-24 py-10 ">
                <div>
                    <table class="w-full table-auto lg:table-fixed">
                        ...
                        <tbody>
                            @foreach ($magazinesubscriptions as $index => $magazinesubscription)
                            
                            <tr>
                                <td class="border  text-[12px] px-4 py-4  w-[100px] text-center">{{ $index + 1 }}</td>    
                                <td class="border  text-[12px] px-4 py-4 text-center  ">{{ $magazinesubscription['name'] }}</td>
                                <td class="border  text-[12px] px-4 py-4 text-center  ">{{ $magazinesubscription['email'] }}</td>
                                <td class="border  text-[12px] px-4 py-4 text-center  ">{{ $magazinesubscription['subscription'] }}</td>

                                <td class="border text-[12px]  px-4 py-4 text-center  ">
                                        {{ $magazinesubscription['created_at']->format('d-m-Y') }}
                                      </td>
