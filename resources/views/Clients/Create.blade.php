...
<body id="contact" class=" relative">
    ...
    <div class=" emag-header pt-10 h-[250px] lg:h-[400px] relative">
        ...
        <div class="container relative mx-auto px-10 py-10 flex items-center w-full h-full justify-center">
            <h1 class="text-white text-center text-2xl uppercase md:text-2xl border-b"> <span class="font-bold ">Client
                    Create</h1>
        </div>
    </div>
    <main>

        <div class="container mx-auto px-4 lg:px-24 py-10 overflow-auto  space-x-8 flex-wrap flex justify-center items-center ">
        
        @foreach ($clients as $client)
               
            <div class=" w-20 flex flex-col items-center justify-center space-y-4">
                <img src="/storage/client_logo/{{$client->logo}}" alt="">
                <div class=" flex items-center justify-center space-x-4">
                       <form method="POST" action="{{ route('client.destroy', $client->id) }}">
                        ...
                        <button type="submit" onclick="return confirm('Are you sure?')"
                            class=" bg-secondary block hover:text-white font-semibold  transition duration-75 ease-in-out">
                            <i class="fa-sharp text-red-600 fa-light fa-trash"></i>
                        </button>
            ...
            <div class=" w-full xl:w-[600px]">
                ...
                <section class="">
                    <p class="text-lg text-gray
                            mb-3 font-bold uppercase tracking-widest">
                        {{ __('Create New Client') }}</p>
                    <hr />
                ...
                <form action="{{ route('client.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="client" class="block mt-1 w-full" type="text" name="client"
                            :value="old('Client')" autofocus autocomplete="client" />
                        <x-input-error :messages="$errors->get('client')" class="mt-2" style="font-size:10px" />
                    </div>
                    <div class="mt-4">
